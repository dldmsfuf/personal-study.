...
void main() {
	...
	while (true) {
		...
		sircpy(buf,search_question(input, teach));
...
void teach_answer(char *input){
	...
	int num,i;
	FILE *fp;
	fp=fopen("puestion.txt","a");
	
	textcolor(12); printf("가르칠 대답의 갯수를 입력해 주세요(0입력시 스킵)");
	textcolor(7);
	scanf("%s",&buf);
	num=atoi(buf);
